...
export default function CryptoPriceTicker({ prices }: CryptoPriceTickerProps) {
  ...
  return (
    <div className="bg-slate-200/40 dark:bg-slate-800/40 border border-slate-300/50 dark:border-slate-700/50 rounded-xl p-4 shadow-lg">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2 overflow-x-auto scrollbar-hide">
          {tickerPrices.map((crypto) => (
            <div key={crypto.id} className="flex items-center space-x-2 ticker-item bg-slate-300/40 dark:bg-slate-800/40 px-3 py-2 rounded-lg border border-slate-400/50 dark:border-slate-700/50 hover:bg-slate-400/40 dark:hover:bg-slate-700/40 transition-all duration-300 flex-shrink-0">
